...
class DeepMDXGAN(pl.LightningModule):
    def __init__(self,
                 batch_size=4,
                 num_workers=12,
                 nout=32,
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        if optimizer_idx == 0:
            ...
            if self.train_discriminator:
                ...
                # loss_real = self.discriminator_criterion(pred_real - pred_fake.mean(0, keepdim=True), torch.ones_like(pred_real))
                # loss_fake = self.discriminator_criterion(pred_fake - pred_real.mean(0, keepdim=True), torch.zeros_like(pred_fake))
                d_loss = (loss_real + loss_fake) / 2
                d_loss = d_loss * self.d_loss_scale
        ...
        if optimizer_idx == 1:
            ...
            pred_real = self.model.discriminator_forward(y).detach()
            ...
            if self.train_discriminator:
                ...
                # adv_loss = self.discriminator_criterion(pred_fake - pred_real.mean(0, keepdim=True), torch.ones_like(pred_fake))
